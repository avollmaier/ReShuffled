...
/**
 * A <tt>LogOutputStreamHandler</tt> object takes log messages from a
 * <tt>Logger</tt> and prints them to an output stream.<br>
...
public class LogOutputStreamHandler extends Handler {
    ...
    /**
     * Constructor. Use system property
     * {@code logging.LogOutputStreamHandler.showRecordHashcode} to enable
     * printing out class name and hashcode of any log record. If this property
     * is not set or set to value false, then these record details are not
     * printed out
     *
    ...
    public LogOutputStreamHandler(OutputStream out) {
        ...
        if (out instanceof PrintStream) {
            this.out = new LogOutputPrintStream(out);
     ...
     *
     * @return true when class-name and hashcode of log record should is printed
     * out
     */
     ...
     * The internal error state is set to <code>true</code> when the underlying
     * output stream throws an {@link Exception}
     *
    ...
    private class LogOutputPrintStream extends PrintStream {
        ...
        public void startBlock(String name, int leftIntend, int length) {
            ...
            for (int i = 0; i < (blockLength - 10 - blockName.length() - leftIntend); i++) {
                print('-');
            }
            println();
        }
        ...
        public void write(byte[] buf, int off, int len) {
            for (int i = 0; i < len; i++) {
                ...
                if (lineFeedDone && blockActive) {
                    for (int j = 0; j < blockLeftIntend; j++) {
                        write(' ');
                    }
                }
                write(b < 0 ? b + 256 : b);
