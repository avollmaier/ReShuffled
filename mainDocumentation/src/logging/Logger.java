 ...
 * instance of the class {@link Level}.</p>
 * <p>
 * The log level can also be dynamically changed by calls on the
 * {@link #setLevel(Level)} method. The {@link Level} class supports a number of
 * predefined Levels ({@link Level#SEVERE}, {@link Level#WARNING}, ... ,
 * {@link Level#FINEST}). These predefined values could be enlarged by extending
 * the class {@link Level}.</p>
 * <p>
 * <br>
 ...
 * {@link LogRecord} class. In this case the location is printed out by the
 * Handler.<br>
...
public class Logger {
    ...
    private static Level parseLevel(String level) throws IllegalArgumentException {
    ...
    private void fireLogRecord(Level l) {
        if (isLoggable(l)) {
            LogRecord r = showLocation ? new ExtendedLogRecord(l, 4) : new LogRecord(l, null);
            logger.log(r);
        }
    }
    ...
    private void fireLogRecord(Level l, int stackTraceDepth, Throwable th, LogRecordData data) {
        if (isLoggable(l)) {
            logger.log(new ExtendedLogRecord(l, showLocation ? 4 : -4, stackTraceDepth, th, data, null));
        }
    }
