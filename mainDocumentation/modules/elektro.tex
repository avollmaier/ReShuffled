...
Der elektrische Teil dieser Diplomarbeit umfasst das Entwerfen und das Entwickeln eines Schaltplans und einer dazugehörigen Leiterplatte sowie das Layouten dieser und deren Bestückung.
Eine Inbetriebnahme und das Durchführen von Tests soll ebenfalls vorgenommen werden.
Die Platine, gesteuert vom Mikroprozessor Atmega324PA, hat die Aufgabe, drei kapazitive Sensoren einzulesen,
...
\footfullcite[Vgl.][]{AVR-Typen}Dieser Mikrochip besitzt einen Flash-Speicher von 16KiB, einen \acs{SRAM} in einer Größe von 1KiB und einen \acs{EEPROM}-Speicher mit 512 Bytes.
...
Wie in \autoref{fig:H-Bridge} zu erkennen ist, befindet sich vor jedem Basiswiderstand der NPN-Transistoren ein UND-Gatter, mit dem jeweils ein digitales Steuersignal und das PWM-Signal zusammengeführt werden.
...
Um ein Beschädigen der Transistoren zu vermeiden, verwendet man diese Dioden, da diese dem Strom einen Pfad bieten, welcher die Energie freisetzt. \\
...
\begin{figure}[ht]
    ...
    \label{fig:ZweiHBruecken}
\end{figure}
\newpage
In der \autoref{fig:ZweiHBruecken} wird eine konkrete Verbindung von zwei H-Brücken in Kombination mit einem Mikrochip dargestellt.
...
Mithilfe des Timer 0 wird am Ausgang OC1B des Mikrocontrollers ein PWM-Signal erzeugt, unter Verwendung dessen,
abhängig vom gewählten Duty-Cycle, Spannungssignale in gewissen Zeitabständen geschickt werden.
Dadurch befindet sich jeweils an einem Eingang der beiden UND-Gatter, mit den Bezeichnungen IC1 und IC2, ein Signal mit einem HIGH-Pegel (Spannung) oder LOW-Pegel (keine Spannung).
...
Am Ausgang PD0 muss ein 0V-Signal zur Verfügung gestellt werden, da ansonsten der PNP-Transistor Q2 sperrt und kein Stromfluss gewährleistet wäre.
...
\subsubsection{A4988}
\footfullcite[Vgl.][]{A4988-Stepper-Motor-Driver-Carrier}Das Modul kann mit einer Eingangsspannung von 8V bis 35V arbeiten und kann ohne den Einsatz eines Kühlkörpers bis zu 1A pro Phase liefern.
...
\footfullcite[Vgl.][]{DRV8825-Stepper-Motor-Driver-Carrier-High-Current}Diesem Treiber-Board von Texas Instruments ist es möglich, mit einer Spannung von 8.2V bis 45V zu arbeiten.
Er kann ohne eine bestimmte Kühlung bis zu 1.5A pro Phase liefern.
...
\footfullcite[Vgl.][]{TB6600-Arduino-Stepper-Motor-Driver}Dieser Ein-Achsen-Schrittmotortreiber ist für hybride Schrittmotoren mit 2 oder 4 Phasen geeignet.
...
Der Ausgangsstrom ist mittels \acs{DIP}-Schalter über acht Schritte wählbar.
...
\footfullcite[Vgl.][]{Induktiv-Kapazitiv-Optisch-oder-Ultraschall}Ein optischer Sensor sendet über seine eigene Lichtquelle einen Lichtstrahl aus.
...
Optische Sensoren besitzen einen fast wartungsfreien, langfristigen Betrieb, da die Detektion kontaktlos erfolgt.
...
\footfullcite[Vgl.][]{Induktiv-Kapazitiv-Optisch-oder-Ultraschall}Die meisten Ultraschall-Sensoren arbeiten nach dem Prinzip der Laufzeitmessung von hochfrequenten Schallimpulsen.
...
So ist es dieser Art von Sensor möglich, unterschiedlichste Materialien wie Metall oder Holz aufzufassen.
Lediglich schalldämpfende Materialien können nicht erfasst werden.
Diese Art der Sensorik ist dazu in der Lage, berührungslos Objekte zu erkennen und ihre Entfernung zum Sensor zu messen.
Ein fast wartungsfreier Betrieb ist möglich.

\subsubsection{Kapazitive Sensoren}
\footfullcite[Vgl.][]{Funktionsweise-und-Technologie-von-kapazitiven-Sensoren}Dringt das zu erfassende Objekt in das Messfeld des kapazitiven Sensors ein, verändert sich somit das elektrische Feld vor der aktiven Fläche des Sensors.
Dadurch ändert sich die Kapazitität eines internen RC-Oszillators, welcher folgend das Schaltkriterium liefert.
...
Somit fiel unsere Wahl auf den LJC18A3, welcher mit einer Versorgungspannung von 6V bis 36V arbeitet und metallische sowie nichtmetallische Gegenstände in einem Schaltabstand von 1 bis 10mm erfassen kann.
...
Aufgrund der Tatsache, dass der Mikrocontroller mit einer Versorgung von 5V gleichzeitig mit einer Logik von 5V arbeiten würde und somit ein Pegelwandler zu 3.3V nötig wäre, verwarfen wir die Idee diesen mit 5V zu versorgen.
...
\newpage
\footfullcite[Vgl.][]{NCP1117-Datasheet}Um zu überprüfen, ob das 3.3V-Netz des Raspberry Pi auch im Stande dazu ist, diese Leistung zur Verfügung zu stellen, wird dessen Datenblatt herangezogen.
...
\footfullcite[Vgl.][]{Verpolschutz}Die einfachste Methode diesen Schutz zu gewährleisten, ist über eine Schottkydiode.
...
Um diese Selbsthaltung wieder zu lösen, benötigt es eine Interaktion mit dem LCD.
...
\footfullcite[Vgl.][]{EEVblog-262-Worlds-Simplest-Soft-Latching-Power-Switch-Circuit}Die folgende Variante kam ebenfalls im Laufe der Diplomarbeit auf.
...
\footfullcite[Vgl.][]{LM2576-Datasheet}Als Buck-Converter wählten wir den LM2576-5.0 aus.
...
Als Basis für unser Human Machine Interface wählten wir einen Raspberry Pi 3B+ aus.
...
\footfullcite[Vgl.][]{Datasheet-ATmega324PA}Um den Mikrocontroller zu versorgen, wurde an allen VCC-Pins und am AVCC-Pin eine Verbindung zum 3.3V Ausgang des Raspberry Pi vorgesehen.
...
\footfullcite[Vgl.][]{DRV8825-Datasheet}Dieses Modul ist dafür ausgelegt, bipolare Schrittmotoren anzusteuern.
...
\footfullcite[Vgl.][]{DRV8825-Datasheet}Der DRV8825 besitzt 16 Anschlüsse.
...
\footfullcite[Vgl.][]{DRV8825-Datasheet}In der folgend zu sehenden Tabelle sind die alle Pegelkombinationen der Modus-Anschlüsse dargestellt.
...
\begin{figure}[ht]
    ...
    \end{circuitikz}
    \caption{Aufbau des Spannungsteilers}
\end{figure}
...
Durch diese Werte ergibt sich bei einer HIGH Signalausgangsspannung des Sensors von 6.92V eine Spannung am Ausgang des Spannungsteilers von 3.31V .
...
\footfullcite[Vgl.][]{Datasheet-CH340G}Dieser Treiberbaustein ist wie folgt zu beschalten:
...
\footfullcite[Vgl.][]{FDD6637-Datasheet}Als P-Kanal MOSFET traf unsere Wahl auf den FDD6637.
...
Der FDD6637 besitzt ein \acs{TO}-252-Gehäuse.

\subsubsection{N-Kanal MOSFET}

\footfullcite[Vgl.][]{FDN337N-Datasheet}Als N-Kanal MOSFET wählten wir den FDN337N aus, welcher in der Lage ist, Ströme von bis zu 2.2A treiben zu können.
...
Der FDN337N besitzt eine Gate-Source-Maximalspannung von 8V und ist mit einem \acs{SOD}-23 Gehäuse erhältlich.

\subsection{Auswahl der Gleichspannungswandler-Peripherie}

\subsubsection{Spule}
\footfullcite[Vgl.][]{DR127-101-R-Datasheet}Um eine geeignete Spule für den Step-Down Wandler auszuwählen, wurde auf dessen Datenblatt zurückgegriffen.
...
\footfullcite[Vgl.][]{MBRD340-Datasheet}Bei der Wahl einer passenden Schottky-Diode wurde ebenfalls auf die technische Dokumentation des Gleichspannungswandlers zurückgegriffen.
...
\footfullcite[Vgl.][]{RR264M-Datasheet}Als diese Dioden selektierten wir die RR264M, welche einem Strom von 1A standhalten können, was wichtig für die Implementierung an den induktiven Lasten ist.
...
\footfullcite[Vgl.][]{MBR130-Datasheet}Als Schottky-Diode wählten wir die MBR130, welche die Maximalwerte von 1A und 30V mit sich bringt.
...
Sie verfügt über eine maximale Verlustleistung von 250mW, eine Durchlassspannung von 0.9V und ein \acs{SOT}-23 Gehäuse.
...
Bei Kondensatoren im µF-Bereich wurde auf SMD-Elektrolytkondensatoren zurückgegriffen.
...
Sobald die Auswahl der Peripherie beendet war, galt es, ein Design für die geplante Platine in KiCad zu entwerfen.
...
\begin{figure}[hb]
    \centering
    \includegraphics[scale=0.095]{fig/elektro/PrototypTop.png}
    \caption{Draufsicht der Oberseite}
\end{figure}
\\
\begin{figure}[hb]
    \centering
    \includegraphics[scale=0.095]{fig/elektro/PrototypBot.png}
...
\begin{itemize}
    ...
    \item D1 als ACPDQC5V0USP-HF
\end{itemize}

\footfullcite[Vgl.][]{Bourns2017}Die wiederverwendbare Sicherung MF-MSMF200 hält eine maximale Spannung von 8V und einen Haltestrom von 2A bei 23°C Umgebungstemperatur aus.
...
\newpage
\footfullcite[Vgl.][]{ACPDQC5V0USP-HF}Die als SMD ausgeführte Suppressordiode ACPDQC5V0USP-HF bringt eine Breakdown Voltage von mindestens 5.4V mit sich.
Sie verfügt über einen niedrigen Leakage-Strom von 1µA bei 25°C und hält einer Spitzenimpulsverlustleistung von 50W stand.
