...
\begin{figure}[ht]
    \centering
    \begin{circuitikz}[european, scale = 1]
        \draw (1.2,2) to [R, l=$R1$](2.5,2) to (3.5,2) to [eC, l=$C1$, *-*](3.5,0);
...
\begin{equation*}
    U_e = RC \cdot \dot u_c + u_c \laplace \frac{U_e}{s} = RC \cdot u_c \cdot s + u_c
\end{equation*}
...
\begin{equation*}
    u_c = U_e \cdot \frac{1}{s \cdot (1 + \tau s)} \Laplace {u_c = U_e \cdot (1 - e^{\frac{-t}{\tau}})}
\end{equation*}
...
Bei einem von uns ausgewähltem Widerstandswert von 560 Ohm und jener Eingangsspannung erschließt sich der Wert des Kondensators nach 0.5 Sekunden wie folgt:
...
\begin{align*}
    ...
    K \cdot e^{\lambda \cdot t} &= K \cdot \lambda \cdot e^{\lambda \cdot t} \\
    \lambda + \frac{1}{RC} &= 0 \\
    \lambda &= -\frac{1}{RC} \\
    u_c &= K \cdot e^{-\frac{t}{RC}}, (t >= 0, K = u_c(t=0)) \\
\end{align*}
...
wobei
\begin{itemize}
    \item P\textsubscript{I} dem maximal fließenden Strom in Ampere,
    \item V\textsubscript{K} 0.048 für externe oder 0.024 für interne Leiterbahnen,
    \item I\textsubscript{dT} dem Temperaturanstieg über der Umgebungstemperatur in Grad Celsius,
    \item V\textsubscript{W} der Leiterbahndicke in mils und
    \item I\textsubscript{H} der Leiterbahnbreite in mils entspricht.
\end{itemize}
