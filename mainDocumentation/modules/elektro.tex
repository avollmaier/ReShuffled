...
\subsubsection{Variante, welche im Laufe der Diplomarbeit aufkam}

\subsubsection{Realisierte Variante mit einzelnen Taster zum Ein- und Ausschalten}


12V, welche vom Netzteil zur Verfügung gestellt werden, werden durch eine Betätigung von T\textsubscript{1} durchgeschaltet.
Durch jene Betätigung von T\textsubscript{1} fließt Strom durch R\textsubscript{1} und aktiviert somit den Bipolartransistor Q\textsubscript{1}.
Dieser Transistor schaltet somit die am Kollektor anliegende Leitung auf Masse.
Das dadurch auf Ground gezogene Gate lässt den P-Kanal MOSFET durchschalten.
Über den Ausgang 12V\textsubscript{OUT} werden somit sämtliche 12V Sensoren, Aktoren und Elektronikkomponenten versorgt.
Das dadurch ebenfalls versorgte Gate von Q\textsubscript{4} zieht die davon am Drain liegende Leitung auf Masse.
Die Spannungsversorgung von 12V wird somit auch nach dem Betätigen von T\textsubscript{1} bereitgestellt.
Um diese Selbsthaltung wieder zu lösen, benötigt es ein digitales „high“ von 3.3V, ausgehend von Pin XY des Mikrokontrollers.
Das dadurch versorgte Gate von Q\textsubscript{5} lässt diesen auf Masse durchschalten und zieht somit die 12V Leitung auf Masse.
Ein genaue Dimensionierung von Cx und Rx sorgen für ein längeres „High“ der Leitung, sodass ein zuverlässiges Abschalten bereitgestellt werden kann.
Der dadurch deaktivierte Q\textsubscript{4} löst die Selbsthaltung und die Schaltung befindet sich wieder in ihrem Grundzustand.
...
\begin{figure}[ht]
    ...
    \begin{circuitikz}[european, scale = 1.2]
        ...
        \draw (7.5,7) to [R, l=$R6$, *-](7.5,5) to (7.5,5) node[ground]{};
        \draw (9,7) to [eC, l=$C2$, *-](9,5) to (9,5)node[ground]{};
...
\subsection{Peripherie des DC/DC Wandlers}
