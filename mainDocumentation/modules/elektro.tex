...
Da unser finales Konzept Zahlreiches an Logikansteuerungen benötigt, ist es sinnvoll die Ansteuerung der Peripherie auf einen Mikrocontroller auszulagern.
...
Dieser Mikrochip besitzt einen Flash-Speicher von 16kiB, einen SRAM in einer Größe von 1kiB und einen EEPROM-Speicher von 512 Bytes.
...
Er verfügt über 32 I/O Register, eine SPI Schnittstelle, eine I2C Schnittstelle, zwei UART Schnittstellen und drei flexible Timer.
...
Er besitzt zwei UART-, eine SPI- sowie eine I2C-Schnittstelle.
...
Um jenen, im Automaten verbauten, Schrittmotor ansteuern zu können, ist eine Baugruppe notwendig.
...
Der von uns ausgewählte Schrittmotor 12HS19-2004S1 ist bipolarer Art und weist 2 Spulen auf, wodurch er sich von zwei H-Brücke steuern lässt.
...
Mithilfe des Timer 0 wird am Ausgang OC1B des µC ein PWM-Signal erzeugt, unter Verwendung dessen, abhängig vom gewählten Duty-Cycle, Spannungssignale in gewissen Zeitabständen geschickt werden.
...
Am Ausgang PD0 muss ein LOW-Signal zu Verfügung gestellt werden, da ansonsten der PNP-Transistor Q2 sperrt und kein Stromfluss gewährleistet wäre.
...
\subsubsection{A4988 \footfullcite{A4988 Stepper Motor Driver Carrier}}
...
Zusätzlich wird die Gewährleistung eines Überstrom- und Übertemperaturschutzes sowie eine Überspannungsabschaltung versichert.
...
\subsubsection{DRV8825\footfullcite{DRV8825 Stepper Motor Driver Carrier, High Current}}
...
Ausgelegt ist dieses Modul auf einen Strom von 2,5A, welches mit zugeführter Kühlung auch getrieben werden kann.
...
\subsubsection{TB6600\footfullcite{TB6600 arduino Stepper Motor Driver}}
...
\subsection{Detektion der Kartenposition\footfullcite{Funktionsweise und Technologie von kapazitiven Sensoren}}
...
\subsubsection{Ultraschall-Sensoren\footfullcite{Induktiv, kapazitiv, optisch oder Ultraschall}}
...
Dieser Sensor konnte mit einem sehr geringen Preis in unseren Besitz gebracht werden und zeigte schon bei den ersten Tests, dass er seiner Aufgabe gewachsen war.
...
\subsubsection{Verpolungsschutz mit einer Diode\footfullcite{Verpolschutz}}
...
\subsubsection{Verpolungsschutz mit einer Sicherung und einer Diode\footfullcite{Verpolschutz}}
...
\subsubsection{Verpolungsschutz mithilfe eines P-Kanal MOSFET\footfullcite{Verpolschutz}}
...
Die Versorgung soll durch ein Signal des RPI gestoppt werden, wodurch ein korrektes Herunterfahren des RPI sichergestellt werden soll.
...
\subsubsection{Andere Variante \footfullcite{EEVblog 262 - World's Simplest Soft Latching Power Switch Circuit}}
...
\subsection{DC/DC Wandler\footfullcite{LM2576 Datasheet}}
...
\subsection{Beschaltung des ATmega324P\footfullcite{Datasheet ATmega324P/V}}
...
\subsection{DRV8825-Schrittmotortreiber\footfullcite{DRV8825 Stepper Motor Driver Carrier, High Current}}
...
Diese hat die Aufgabe, den MOSFET vor einer selbsterzeugten, rückfließenden Spannung des Hubmagneten zu schützen.
...
\subsection{Mini-USB Schnittstelle\footfullcite{Datasheet CH340G}}
...
\subsubsection{P-Kanal MOSFET \footfullcite{FDD6637 Datasheet}}
...
\subsubsection{N-Kanal MOSFET \footfullcite{FDN337N Datasheet}}
...
\subsubsection{Spule \footfullcite{DR127-101-R Datasheet}}
...
einen niedrigen Widerstandswert, magnetische Geschirmtheit und ein SMD-Gehäus mit sich bringt.

\subsubsection{Schottky-Diode\footfullcite{MBRD340 Datasheet}}
...
\subsection{Auswahl der Gleichrichter-Dioden\footfullcite{RR264M Datasheet}}
...
\subsection{Auswahl der Schottky-Dioden\footfullcite{MBR130 Datasheet}}
...
\subsection{Auswahl der Zener-Diode\footfullcite{BZX84 series Datasheet}}
...
\subsubsection{Crowbar Circuit\footfullcite{The Art Of Electronics - Kapitel 9.13.1 Overvoltage crowbars}}
...
Der SCR wird leitend, sobald der Wert der Versorgungsspannung jenen nominalen Zenerspannungswert der Zenerdiode und die Gate-Triggerspannung des Thyristors übersteigt.
...
\begin{figure}[htp]
    \centering
    \begin{circuitikz}[european, scale = 1]
        \draw (5,3) to [/tikz/circuitikz/bipoles/length=1.1cm, D-, l_=$D_1$](5,6) to (5,6)node[vcc]{};
        ...
        \draw (2,3) to [R, l=$R_1$](4,3) to [short, -*](4,3) to [short, -*](5,3);
        \draw (4,3) to [C, l_=$C_1$, -*](4,0);
        \draw (5,0) to [/tikz/circuitikz/bipoles/length=1.1cm, D-, l_=$D_2$,](5,3);
        \draw (5,3) to [R, l=$R_2$](7,3);
