...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{fig/ainf/SceneBuilder.png}
...
Nachfolgend schließt das Trennzeichen (\verb!#!) diese Daten ab.\\\\
...
Wichtig beim CRC-Verfahren ist, dass beide Teilnehmer, also Sender und Empfänger, dasselbe Schema zur Berechnung der Prüfsumme verwenden.
Hierbei spielt der Grad des Generator-Polynoms sowie die mögliche Invertierung eine wesentliche Rolle.
...
\lstinputlisting[firstline=90, lastline=102, style=java,caption=Methode createRequestFrame(),label=Gesamte Methode createRequestFrame()]{src/serial/request/Request.java}
...
Umgesetzt wird diese mithilfe eines sogenannten \lstinline[style=java]{PipedInputStream} und \lstinline[style=java]{PipedOutputStream}.
...
Die Daten werden ab jetzt nicht mehr in der standard Darstellung angezeigt, sondern basieren auf einem eigens dafür angepassten Rendering.
Solch ein Verfahren hat den Vorteil, dass nun direkt ein Objekt in die ComboBox bzw. in die Liste importiert werden kann und ab diesem Zeitpunkt die Referenzierung des selektierten Elements leichter fällt.
Auch werden hierbei Ressourcen eingespart, denn es werden nur die Elemente gerendert, die auch wirklich angezeigt werden.
...
\lstinputlisting[firstline=43, lastline=45, style=java,caption=Initialisierung der Achsen,label=XYChart]{src/gui/controller/HomeController.java}
Nachdem diese Deklarierung erfolgte, kann mit der Methode \lstinline[style=java]{updateBarChart()} die Grafik aktualisiert werden.
...
\lstinputlisting[firstline=43, lastline=56, style=java,caption=createTreeData(),label=createTreeData]{src/gui/controller/StatsController.java}
...
\lstinputlisting[firstline=58, lastline=60, style=java,caption=createGroupTreeItem(),label=createGroupTreeItem]{src/gui/controller/StatsController.java}
...
\lstinputlisting[firstline=90, lastline=101, style=java,caption=activateLocale(),label=multiLang]{src/gui/multilanguage/ResourceManager.java}
...
Um nun die gesendeten Daten über der seriellen Schnittstelle auf der Ansteuerplatine zu erhalten ist es von Nöten, diese genannte Platine einzurichten und zu programmieren.
...
So werden beispielsweise die ATmega Prozessoren mit aktiviertem internen Oszillator ausgeliefert und man darf sich dementsprechend nicht wundern, wenn der µC einfach nicht mit dem angeschlossenen Quarz (XTAL) oszillieren will.
...
\lstinputlisting[firstline=142, lastline=176, style=c,caption=Funktion apphandleUartByte(),label=multiLang]{src/app.c}
...
Nach dem Anschließen muss nur mehr die \textit{config.txt} am Betriebssystem des Raspberry PI ergänzt werden.
