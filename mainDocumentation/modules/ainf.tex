...
Im Allgemeinen besteht das Ziel darin, eine benutzerfreundliche grafische Bedienoberfläche zu realisieren, welche schlussendlich im Betrieb auf einem 7" Display angezeigt werden soll.
...
Dadurch muss, bevor mit dem Programmierprozess gestartet wird, eine passende Arbeitsumgebung ausgewählt werden, welche alle vom Benutzer gestellten Anforderungen erfüllt.
...
Das etwas veraltete und lieblose Look and Feel von Java-Swing brachte die Entscheidung, den Nachfolger, nämlich JavaFX, zu verwenden.
...
Der logische Code hinter der GUI befindet sich jedoch nicht in diesen Dateien, sondern in eigenen Controller-Klassen, welche mit der GUI programmatisch verknüpft sind.
...
Im Gegensatz zu Java-Swing muss man als Programmierer hier auch nicht die sogenannte \lstinline[style=java]{Stage}, also das jeweilige Fenster selbst, mithilfe von \lstinline[style=java]{new JFrame()} erzeugen, sondern dies wird von JavaFX automatisch erledigt.
...
Dies bedeutet, dass eine Kapselung des Konstruktionsprozesses innerhalb der Klasse vorliegt.
...
SceneBuilder bzw. JavaFX bietet bereits einige Grundelemente für Layouts an, diese jedoch nicht sehr ansprechend sind.
Sie können zwar mit CSS, was im \autoref{sssec:CSS} beschrieben wird, gestaltet werden.
Dies ist jedoch für alle Elemente schwierig umzusetzen.
...
Auch ist es notwendig, ein Tag mithilfe eines sogenannten End-Tags zu schließen.
...
Wiederum wird anschließend auf die Verarbeitung von Daten vom Simulator gesprochen, denn dies spielt eine wichtige Rolle und sollte nicht unerwähnt bleiben.
...
\lstinputlisting[firstline=7, lastline=16, style=java,caption=fdsa,label=gamemodeModel]{src/data/model/GamemodeModel.java}
...
\lstinputlisting[firstline=43, lastline=45, style=java,caption=fdsa,label=XYChart]{src/gui/controller/HomeController.java}
...
\lstinputlisting[firstline=43, lastline=56, style=java,caption=fdsa,label=createTreeData]{src/gui/controller/StatsController.java}
...
\lstinputlisting[firstline=58, lastline=60, style=java,caption=fdsa,label=createGroupTreeItem]{src/gui/controller/StatsController.java}
...
\lstinputlisting[firstline=62, lastline=73, style=java,caption=fdsa,label=dataBindinh]{src/gui/controller/StatsController.java}
...
\lstinputlisting[firstline=24, lastline=41, style=java,caption=fdsa,label=dataBindinh]{src/util/AlertUtil.java}
...
\lstinputlisting[firstline=17, lastline=20, style=java,caption=fdsa,label=readBundle]{src/util/FileUtil.java}
...
\lstinputlisting[firstline=90, lastline=101, style=java,caption=fdsa,label=multiLang]{src/gui/multilanguage/ResourceManager.java}
...
\footfullcite{mikrocontroller.net2020}Nach der Bereitstellung der Platine beginnt der Prozess der Einrichtig des Mikrocontrollers.
...
\lstinputlisting[firstline=142, lastline=176, style=c,caption=fdsa,label=multiLang]{src/app.c}
